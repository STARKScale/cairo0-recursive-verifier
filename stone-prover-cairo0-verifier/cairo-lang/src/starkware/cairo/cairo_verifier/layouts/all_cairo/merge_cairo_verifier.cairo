...
func main{
    ...
    alloc_locals;
    local proof1: StarkProof*;
    local proof2: StarkProof*;
    %{
        from starkware.cairo.stark_verifier.air.parser import parse_proof
        ids.proof1 = segments.gen_arg(parse_proof(
            identifiers=ids._context.identifiers,
            proof_json=program_input["proof1"],
        ))
        ids.proof2 = segments.gen_arg(parse_proof(
            identifiers=ids._context.identifiers,
            proof_json=program_input["proof2"],
        ))
        

    %}
    let (program_hash1, output_hash1) = verify_cairo_proof(proof1);
    let (program_hash2, output_hash2) = verify_cairo_proof(proof2);

    // Write program_hash and output_hash to output.
    assert [cast(output_ptr[0], CairoVerifierOutput*)] = CairoVerifierOutput(
        program_hash=program_hash1, output_hash=output_hash1
    );

    assert [cast(output_ptr[1], CairoVerifierOutput*)] = CairoVerifierOutput(
        program_hash=program_hash2, output_hash=output_hash2
    );

    let output_ptr = output_ptr + 2*CairoVerifierOutput.SIZE;
